...
function Header() {
  ...
  return (
    <nav className="bg-[#181d20] p-4">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-40 p-4">
        ...
        <div className={`items-center justify-between w-full md:flex md:w-auto md:order-1 ${isMenuOpen ? 'block' : 'hidden'} md:block`}>
          <ul className="flex flex-col font-medium p-4 md:p-0 mt-4  md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 text-[#7f69a5]  md:mx-0"> 
            {['About', 'Skills', 'Projects', 'Education', 'Contact'].map((link, index) => (
              <li key={index}>
                <a
                  href={`#${link.toLowerCase()}`}
                  className={`block py-2 px-3 hover:text-lg md:p-0 `}
